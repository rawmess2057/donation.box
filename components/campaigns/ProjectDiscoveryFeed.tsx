...
export default function ProjectDiscoveryFeed({
  ...
  useEffect(() => {
    if (isHovering || campaigns.length === 0) return;
        ...
        <div className="flex items-end justify-between mb-6">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <button
              ...
              className="rounded-full bg-white/[0.04] backdrop-blur-2xl border border-white/20 p-2 text-fg-muted transition-all duration-300 hover:text-white hover:bg-white/10 hover:border-white/30 disabled:opacity-30 disabled:cursor-not-allowed"
          ...
          {sorted.slice(0, 8).map((campaign, i) => (
            ...
            >
              <Link
                ...
                className="group block w-56 rounded-2xl bg-white/[0.04] backdrop-blur-xl border border-white/10 overflow-hidden hover:shadow-[0_0_30px_rgba(127,191,127,0.12)] hover:border-primary/30 transition-all duration-500"
              >
                <div className="relative h-28 overflow-hidden">
                  ...
                  <span className="absolute bottom-2 left-2 text-white text-[10px] font-semibold bg-black/50 backdrop-blur-sm rounded-full px-2 py-0.5">
                    {campaign.category}
                  </span>
                </div>
